...
Many engineered robots are made from hard materials, because rigid materials are easy to predict and control. However, engineers inspired by biological systems have begun to explore designing and manufacturing systems with soft bodies. The field of soft-bodied robots, or "soft robotics," aims to develop adaptable machines that are less dangerous for human interaction than rigid robots. The robot covered in this paper utilizes both rigid and soft materials to create a robot that can jump by combusting butane and oxygen. As detailed in the paper, the machine achieves locomotion by first inflating some of its legs to shift the body in the intended jump direction. Once in position the explosive actuator (located at the bottom of the robot) combusts and inflates the bottom hemispheroid, which in turn pushes against the ground and launches the robot. 
